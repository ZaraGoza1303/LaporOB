...
export class TugasService implements ITugasService {
    ...
    async getAll(kategoriId?: string): Promise<TugasDetailRes[]> {
        try {
            ...
            const rawData = await this.tugasRepo.getAll(kategoriId);
            const data: TugasDetailRes[] = rawData.map((tugas: TugasDetailPayload) => {
    ...
    async create(req: CreateTugasReq): Promise<void> {
        try {
            const tugasReq: TugasCreateInput = {
                ...
                ...(req.lantai_id && { lantai: { connect: { id: req.lantai_id } } }),
    ...
    async update(tugasId: string, req: UpdateTugasReq): Promise<void> {
        try {
            ...
            if (req.lantai_id !== undefined) {
                if (req.lantai_id === null) {
                    tugasReq.lantai = { disconnect: true };
                } else {
                    tugasReq.lantai = { connect: { id: req.lantai_id } };
                }
            }
            if (req.ob_id !== undefined) {
                if (req.ob_id === null) {
                    tugasReq.ob = { disconnect: true };
                } else {
                    tugasReq.ob = { connect: { id: req.ob_id } };
                }
            }
            if (req.catatan !== undefined) tugasReq.catatan = req.catatan;
            if (req.status !== undefined) {
                tugasReq.status = req.status;
                const now = new Date();
                if (req.status === TUGAS_STATUS.SEDANG_DIKERJAKAN) {
                    tugasReq.dikerjakan_at = now;
                } else if (req.status === TUGAS_STATUS.SELESAI) {
                    tugasReq.selesai_at = now;
                }
            }
            ...
            if (existing.kategori_id) await this.redis.del(`tugas:all:${existing.kategori_id}`);
            await this.redis.del("tugas:all:all");
    ...
    async approveTugas(tugasId: string, adminId: string): Promise<void> {
        try {
            ...
            if (tugas.kategori_id) await this.redis.del(`tugas:all:${tugas.kategori_id}`);
            await this.redis.del("tugas:all:all");
        } catch (err) {
            if (err instanceof AppError) throw err;
            throw handlePrismaError(err);
